...
train_loader = DataLoader(
    RolloutSequenceDataset(thread_dirs, SEQ_LEN, transform, buffer_size=30),
    batch_size=BSIZE, num_workers=8, shuffle=True, drop_last=True)
test_loader = DataLoader(
    RolloutSequenceDataset(thread_dirs, SEQ_LEN, transform, train=False, buffer_size=10),
...
def to_latent(obs, next_obs):
    ...
    with torch.no_grad():
        obs, next_obs = [
            f.upsample(x.view(-1, 3, 96, 96), size=RED_SIZE,
